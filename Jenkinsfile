node {
   ...
   ///////////////////////////////////////////////
   def projectName= "shs-addyourproject-sys-api" // add project name . example - shs-address-sys-api
   ...
   stage('Preparation') { // for display purposes
      ...
      sh 'echo $BRANCH_NAME'
      git url: "git@github.com:SearsHomeServices/${projectName}.git"
      sh "git clean -f"
      sh "git reset --hard"
   ...
   withCredentials([  [$class: 'StringBinding', credentialsId: 'hs_mvn_version', variable: 'HSMAVEN']]) {
   stage('Build') {

	  
      if ("$BRANCH_NAME"!='master') {
	withMaven(maven : "${HSMAVEN}", mavenSettingsConfig: 'aef5450c-39ae-4804-924a-cd7e30ad7fd7',
        mavenLocalRepo: '.repository') {
         ...
         stage('tag') {
            sh "git tag -f ${projectName}-1.0.0-SNAPSHOT -m 'JenkinsRelease'"
   ...
   stage('Deploy') {
	  ...
	  [$class: 'StringBinding', credentialsId: "${muleenv}", variable: "ENV"],
	    ...
	    println "Starting Maven Deploy..."
		  ...
		  sh 'mvn --batch-mode mule:deploy "-Dartifact=target/'+readMavenPom().getArtifactId()+'-'+readMavenPom().getVersion()+'-mule-application.jar" "-DbusinessGroup=${BUSINESS_GROUP}" "-Dmule.env=${ENV}" "-Dregion=${WORKER_REGION}" "-DworkerType=${WORKER_TYPE}" "-Dworkers=${WORKER_AMOUNT}" "-Dmule.environment=${MULE_ENVIRONMENT}" "-DapplicationName='+readMavenPom().getArtifactId()+'-${ENV}" "-Dusername=${ANY_USERNAME}" "-Dpassword=${ANY_PASSWORD}" "-Dvault.key=${MULE_KEY}" "-Dclient.id=${CLIENT_USERNAME}" "-Dclient.secret=${CLIENT_PASSWORD}" '
	    }
         }
   }}
